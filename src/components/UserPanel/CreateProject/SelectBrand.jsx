...
const SelectBrand = ({
  ...
  useMemo(() => {
    if (!projectData?.brandKit) {
      if (data?.length > 0) {
        const isExist = data?.find(
          (item) => item?._id === projectData?.brandKit
        );
        setSelectedBrand({
          brandName: isExist?.brandName,
          _id: isExist?._id,
          brandLogo: isExist?.brandLogo,
        });
      }
    }
  }, [projectData]);
  ...
  return (
    <div className="border rounded-xl p-6 mb-6">
      ...
      <div className="flex items-center flex-wrap gap-6 mb-6 max-h-[500px] overflow-y-auto">
        <Link to="/user/brand-kit/create" target="_blank">
          ...
          <>
            {data.map((item, index) => (
              <div
                ...
                key={index}
                className="border-indigo-600 border text-center rounded-xl p-2 w-[120px] h-[124px] text-sm font-semibold cursor-pointer"
              >
                <div className="text-end">
                  <input
                    type="radio"
                    checked={item?._id === selectedBrand?._id}
                  />
                </div>
                ...
                <div className="w-[48px] h-[48px] rounded-full mx-auto">
                  <img
                    ...
                    src={viewImg(item?.brandLogo) || playIcon}
                    alt=""
                  />
                </div>
                <p>{item?.brandName}</p>
